const CLEARSPACE_VERSION = '1.3.8';
  ...
  _styleBlock() {
    return `
      <style>
        ...
        .subtitle { display:none; font-size:12px; color:var(--secondary-text-color); }
...
class ClearSpaceCardEditor extends HTMLElement {
  ...
  _renderEntityField() {
    ...
    if (options.length) {
      ...
      const autoChecked = !current ? 'checked' : '';
      const radios = [
        `<label class="picker-option"><input type="radio" data-field="entity" data-radio-value="" name="clearspace-account" ${autoChecked} /> Auto-detect best ClearSpace account</label>`,
        ...optionList.map((option) => `<label class="picker-option"><input type="radio" data-field="entity" data-radio-value="${escapeHtml(option.entityId)}" name="clearspace-account" ${selected?.entityId === option.entityId ? 'checked' : ''} /> ${escapeHtml(option.label)}</label>`),
      ].join('');
      return `
        <div class="picker">
          <div class="picker-count">${optionList.length} ClearSpace account${optionList.length === 1 ? '' : 's'} detected</div>
          <div class="picker-list">${radios}</div>
        </div>
      `;
    }
    ...
    this.querySelectorAll('[data-field]').forEach((input) => {
      ...
      input.addEventListener(eventName, () => {
        const value = input.type === 'radio'
          ? input.getAttribute('data-radio-value')
          : input.type === 'checkbox'
            ? input.checked
            : input.type === 'number'
              ? Number(input.value)
